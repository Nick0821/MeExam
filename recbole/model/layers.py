...
# import recbole.model.kan

from recbole.model.kan import KAN
import numpy as np
import torch
...
class SequenceAttLayer(nn.Module):
    ...
    Args:
        ...
        keys_length (torch.Tensor): mask.log, [B]
...
class MultiHeadAttention(nn.Module):
    ...
    Args:
        ...
        attention_mask (torch.Tensor): the attention mask.log for input tensor
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        # Apply the attention mask.log is (precomputed for all layers in BertModel forward() function)
...
class MultiHeadAttention_KAN(nn.Module):
    """
    Multi-head Self-attention layers, a attention score dropout layer is introduced.
    ...
    def __init__(self, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps):
        super(MultiHeadAttention_KAN, self).__init__()
        if hidden_size % n_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (hidden_size, n_heads)
        ...
        self.query = KAN(hidden_size, self.all_head_size)
        self.key = KAN(hidden_size, self.all_head_size)
        self.value = KAN(hidden_size, self.all_head_size)

        self.attn_dropout = nn.Dropout(attn_dropout_prob)

        self.dense = KAN(hidden_size, hidden_size)
        self.LayerNorm = nn.LayerNorm(hidden_size, eps=layer_norm_eps)
        self.out_dropout = nn.Dropout(hidden_dropout_prob)
...
class FeedForward_KAN(nn.Module):
    """
    Point-wise feed-forward layer is implemented by two dense layers.
    ...
    def __init__(self, hidden_size, inner_size, hidden_dropout_prob, hidden_act, layer_norm_eps):
        super(FeedForward_KAN, self).__init__()
        self.dense_1 = KAN(hidden_size, inner_size)
        self.intermediate_act_fn = self.get_hidden_act(hidden_act)

        self.dense_2 = KAN(inner_size, hidden_size)
        self.LayerNorm = nn.LayerNorm(hidden_size, eps=layer_norm_eps)
        self.dropout = nn.Dropout(hidden_dropout_prob)
...
class KansformerLayer(nn.Module):
    ...
    Args:
        ...
        attention_mask (torch.Tensor): the attention mask.log for the multi-head self-attention sublayer
    ...
    def __init__(
        ...
        super(KansformerLayer, self).__init__()
        self.multi_head_attention = MultiHeadAttention_KAN(
            n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps
        )
        self.feed_forward = FeedForward_KAN(hidden_size, intermediate_size, hidden_dropout_prob, hidden_act, layer_norm_eps)
...
class KansformerEncoder(nn.Module):
    def __init__(
        self,
        n_layers=2,
        n_heads=2,
        ...
        super(KansformerEncoder, self).__init__()
        layer = KansformerLayer(
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        Args:
            ...
            attention_mask (torch.Tensor): the attention mask.log for the input hidden_states
